...
# Características extraídasq
knn.fit(x_train_pca, y_train)
y_pre = knn.predict(x_test_pca)
acc.append(100*accuracy_score(y_test, y_pre))

# Características estimadas y extraídas
knn.fit(x_train_all, y_train)
y_pre = knn.predict(x_test_all)
acc.append(100*accuracy_score(y_test, y_pre))

# Características estimadas SFS
knn.fit(x_train[:,sfs_est], y_train)
y_pre = knn.predict(x_test[:,sfs_est])
acc.append(100*accuracy_score(y_test, y_pre))

# Características estimadas SBS
knn.fit(x_train[:,sbs_est], y_train)
y_pre = knn.predict(x_test[:,sbs_est])
acc.append(100*accuracy_score(y_test, y_pre))

# Características extraídas SFS
knn.fit(x_train_pca[:,sfs_ext], y_train)
y_pre = knn.predict(x_test_pca[:,sfs_ext])
acc.append(100*accuracy_score(y_test, y_pre))

# Características extraídas SBS
knn.fit(x_train_pca[:,sbs_ext], y_train)
y_pre = knn.predict(x_test_pca[:,sbs_ext])
acc.append(100*accuracy_score(y_test, y_pre))

# Características estimadas y extraídas SFS
knn.fit(x_train_all[:,sfs_all], y_train)
y_pre = knn.predict(x_test_all[:,sfs_all])
acc.append(100*accuracy_score(y_test, y_pre))

# Características estimadas y extraídas SBS
knn.fit(x_train_all[:,sbs_all], y_train)
y_pre = knn.predict(x_test_all[:,sbs_all])
acc.append(100*accuracy_score(y_test, y_pre))


#%% Despliegue de resultados

from matplotlib.pyplot import bar,ylim,title,ylabel
